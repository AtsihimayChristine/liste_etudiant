...
function Etudiant(_nom = '', _prenom = '', _sexe = false, _dateNais = '', _lieuNais, _parcours = '', _poids = 0, _taille = 0.0, _matieres = []) {
    this.nom = _nom;
    this.prenom = _prenom;
...
function onSaveButtonClick(matiere) {
    ...
    let _sexe = document.getElementById('sexe').checked;
    let _poids = document.getElementById('txt_poids').value;
    let _taille = document.getElementById('txt_taille').value;

    const Sexe = () => {
        if(_sexe) return "Homme"
        else return "Femme"
    }

    let _student = new Etudiant(_nom, _prenom, Sexe(), _dateNais, _lieuNais, _parcours, _poids, _taille, matiere);
